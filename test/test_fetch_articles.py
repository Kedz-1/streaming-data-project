...
def test_function_returns_a_list(guardian_key, example_search_term):
...
def test_function_returns_message_when_no_articles(
    monkeypatch, guardian_key, example_search_term
):
...
def test_function_returns_up_to_10_articles_when_search_term_present(
    guardian_key, example_search_term, mock_guardian_response, monkeypatch
):
...
def test_function_returns_articles_containing_search_term(
    guardian_key, example_search_term, mock_guardian_response, monkeypatch
):
    ...
    assert all(
        re.search(pattern, x["webTitle"], re.IGNORECASE) for x in result["articles"]
    )


def test_function_does_not_crash_regex_on_special_character(
    guardian_key, mock_guardian_response, monkeypatch
):
...
def test_function_returns_articles_from_a_particular_date(
    guardian_key, example_search_term, example_date, mock_guardian_response, monkeypatch
):
    ...
    web_date = [
        datetime.strptime(x["webPublicationDate"], "%Y-%m-%dT%H:%M:%SZ").date()
...
def test_function_returns_no_articles_when_not_in_date(
    guardian_key, example_search_term, mock_guardian_response, monkeypatch
):
